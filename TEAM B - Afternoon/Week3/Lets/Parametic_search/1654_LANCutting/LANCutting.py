...
# -> 랜선의 길이가 터무니없음. 2^31-1 이면 말도 안되게 큰 수니까 랜선 길이에다가 이분탐색을 조져야겠다 라는 생각을 해야함
...
import sys
sys.stdin = open("1654_LANCutting/LANCutting.txt", "r")
...
while start <= end: # start 가 end 를 넘어가는 순간 이진탐색 종료
    ...
    if cnt < N: # 랜선수가 적게 나왔다? -> 자를려고 시도하는 랜선의 길이를 줄인다 -> mid 값을 낮춰야하니까 start 를 start 로, end 를 mid - 1 로
        ...
        answer = mid # 임시 답안이 나왔을때를 저장을 안해줘서 틀렸었음
        start = mid + 1
